...
const USER_AGENT = 'Mozilla/5.0 (compatible; Chu-kei official IR audit/5.2)';
...
const fetchText = async url => {
  ...
  if (/pdf/i.test(contentType)) return { finalUrl: response.url, html: '', pdfBuffer: Buffer.from(await response.arrayBuffer()) };
  return { finalUrl: response.url, html: await response.text(), pdfBuffer: null };
};
...
const pdfToEvidence = async candidate => {
  ...
  const parsed = await parser(buffer, { pagerender: renderPdfPage, max: 140 });
...
for (const company of companies) {
  ...
  report[company.code] = { name: company.name, startUrl: company.startUrl, visitedPages: [...visited], pageErrors, discoveredPdfCount: pdfCandidates.size, documents };
}

const output = { version: 'final10-static-official-research-v2', generatedAt: new Date().toISOString(), companies: report };
fs.mkdirSync(path.join(ROOT, 'artifacts'), { recursive: true });
fs.writeFileSync(path.join(ROOT, 'artifacts', 'final10-static-official-research-v2.json'), `${JSON.stringify(output, null, 2)}\n`);
...
console.log(JSON.stringify({ companies: companies.length, documents: Object.values(report).reduce((sum, item) => sum + item.documents.length, 0) }, null, 2));

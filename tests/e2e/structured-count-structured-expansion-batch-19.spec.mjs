...
test('keeps at least 218 companies available for structured comparison', async ({ page }) => {
  ...
  expect(structuredCount).toBeGreaterThanOrEqual(218);
});

test('exposes every structured-expansion-batch-19 company through search and detail', async ({ page }) => {
  await page.goto('/');

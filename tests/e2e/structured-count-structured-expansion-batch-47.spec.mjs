...
test('keeps at least 225 companies available for structured comparison', async ({ page }) => {
  ...
  expect(structuredCount).toBeGreaterThanOrEqual(225);
});

test('exposes every structured-expansion-batch-47 company through search and detail', async ({ page }) => {
  await page.goto('/');

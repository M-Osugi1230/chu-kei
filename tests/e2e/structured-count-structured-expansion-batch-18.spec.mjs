...
test('keeps at least 216 companies available for structured comparison', async ({ page }) => {
  ...
  expect(structuredCount).toBeGreaterThanOrEqual(216);
});

test('exposes every structured-expansion-batch-18 company through search and detail', async ({ page }) => {
  await page.goto('/');

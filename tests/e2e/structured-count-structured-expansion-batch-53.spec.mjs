...
test('keeps at least 229 companies available for structured comparison', async ({ page }) => {
  ...
  expect(structuredCount).toBeGreaterThanOrEqual(229);
});

test('exposes every structured-expansion-batch-53 company through search and detail', async ({ page }) => {
  await page.goto('/');

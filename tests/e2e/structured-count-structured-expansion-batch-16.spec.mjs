...
test('keeps at least 211 companies available for structured comparison', async ({ page }) => {
  ...
  expect(structuredCount).toBeGreaterThanOrEqual(211);
});

test('exposes every structured-expansion-batch-16 company through search and detail', async ({ page }) => {
  await page.goto('/');

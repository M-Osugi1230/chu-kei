...
test('keeps at least 227 companies available for structured comparison', async ({ page }) => {
  ...
  expect(structuredCount).toBeGreaterThanOrEqual(227);
});

test('exposes every structured-expansion-batch-50 company through search and detail', async ({ page }) => {
  await page.goto('/');

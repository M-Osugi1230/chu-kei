...
test('keeps at least 231 companies available for structured comparison', async ({ page }) => {
  ...
  expect(structuredCount).toBeGreaterThanOrEqual(231);
});

test('exposes every structured-expansion-batch-54 company through search and detail', async ({ page }) => {
  await page.goto('/');

...
test('keeps at least 233 companies available for structured comparison', async ({ page }) => {
  ...
  expect(structuredCount).toBeGreaterThanOrEqual(233);
});

test('exposes every structured-expansion-batch-55 company through search and detail', async ({ page }) => {
  await page.goto('/');

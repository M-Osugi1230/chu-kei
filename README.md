...
2026-08-01のPhase 0再監査で、旧「3,000社すべて5つ星」を廃止し、掲載件数と深掘り確認済み件数を分離しました。2026-08-15時点ではPhase 2一次レビューWave 38まで完了しています。
...
一次レビュー完了とDeep Verification最終承認は別状態です。自動抽出や候補選定、一次レビュー完了だけで深掘り承認を付与しません。
...
## Development priority after Wave 38

1. Phase 2残り80社をレビューし、Phase 1込み一次レビュー500 / 500社へ到達する
2. 独立図表レビューpending 306社を古いWaveから並行処理する
3. 独立レビュー完了企業からDeep Verification最終承認工程へ進める
4. 610社の決算短信起点データから正式な中計資料を再探索する
5. 116社の証跡候補と6件の404を解消する
6. 353件の進捗データを企業詳細へ同期し、72社・258件の実績接続を維持・拡大する
7. GitHub正本と `chu-kei.com` の品質表示・企業データを完全同期する
8. 新中計・改定・決算更新の日次検知と週次公開を定常化する
9. 500社工程で得た品質指標を反映し、基準を緩めず1,000社へ段階拡張する

...
test('keeps at least 222 companies available for structured comparison', async ({ page }) => {
  ...
  expect(structuredCount).toBeGreaterThanOrEqual(222);
});

test('exposes every structured-expansion-batch-21 company through search and detail', async ({ page }) => {
  await page.goto('/');

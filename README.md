...
実際の氏名、メールアドレス、会社名、案件内容、見積、個別納品物はGitHubへ保存しません。Netlify Formsの受付確認と通知設定はIssue #49で管理します。

## GitHub Actions recovery and local validation

privateリポジトリのActions実行基盤ブロッカーはIssue #45で管理しています。

- 証跡: `operations/ci/actions-blocker-v1.json`
- 復旧手順: `docs/ACTIONS_RECOVERY_RUNBOOK_V1.md`
- ローカル品質ゲート: `scripts/run_local_quality_gate.mjs`

Actions停止中の早期検知には次を使用します。
...
結果は `artifacts/local-quality-gate-v1.json` に保存されます。Actions復旧後も、commit前のローカル検証として残します。

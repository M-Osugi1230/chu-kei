...
test('keeps at least 220 companies available for structured comparison', async ({ page }) => {
  ...
  expect(structuredCount).toBeGreaterThanOrEqual(220);
});

test('exposes every structured-expansion-batch-20 company through search and detail', async ({ page }) => {
  await page.goto('/');

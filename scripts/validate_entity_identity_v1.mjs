...
const milestone=fs.existsSync(MILESTONE)?JSON.parse(fs.readFileSync(MILESTONE,'utf8')):{companyTotal:570};
const companies=data.companies||[],codes=companies.map(c=>String(c.code));
check(`company count ${milestone.companyTotal}`,companies.length===milestone.companyTotal,`actual=${companies.length}`);

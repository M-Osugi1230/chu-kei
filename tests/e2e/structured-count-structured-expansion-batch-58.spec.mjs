...
test('keeps at least 239 companies available for structured comparison', async ({ page }) => {
  ...
  expect(structuredCount).toBeGreaterThanOrEqual(239);
});

test('exposes every structured-expansion-batch-58 company through search and detail', async ({ page }) => {
  await page.goto('/');

...
test('keeps at least 2996 companies available for structured comparison', async ({ page }) => {
  ...
  await expect(page.locator('#stat-confirmed')).toHaveText('2996社');
  const structuredCount = Number((await page.locator('#stat-structured').textContent()).replace(/[^0-9]/g, ''));
  expect(structuredCount).toBeGreaterThanOrEqual(2996);
});

test('exposes every structured-expansion-batch-90 company through search and detail', async ({ page }) => {
  await page.goto('/');
